...
| Sprint 4 — Frontend consolidation | 🟡 in progress (4.1, 4.2 done; 4.3 not done) |
| Sprint 5 — Service layer maturity | 🟡 5.3 done, 5.1/5.2 not done |
| Sprint 6 — Integration testing | 🟡 partial (Sprint 1/2 coverage strong) |

**Highest-value next steps** (by roadmap risk ordering): Sprint 4.3 (pagination on list endpoints + UI) → Sprint 5.1/5.2 (fee generation consolidation + COALESCE subqueries) → Sprint 6.1/6.3 (remaining service tests + smoke tests). No architecture change should proceed directly on `main`.

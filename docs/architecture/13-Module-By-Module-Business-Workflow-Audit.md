...
1. **No "division" concept in admin mental model.** Admins think in "classes"; the codebase now thinks in "divisions + classes". There's no admin-facing page that lists *divisions* — only *classes*. A division is a derived concept.
2. **No color legend.** The Stage B4 palette gives Music an emerald color, but no admin page tells you "emerald = Music". Color is per-tab/button only.
...
4. **Accountant workflows assume two divisions.** The "Students" filter bar (gap #1 above) is the symptom; the root cause is the accountant page treats divisions as static, not dynamic.
5. **No way to see "what business rules are unique to Kirtan" from the admin UI.** The Kirtan Sunday-only rule, the kirtan-score column, and the lesson-notes block are scattered. A "division settings" view would surface them, but is not in scope for Stage B.
...
- **L-2** Cross-division reports (one PDF, all divisions).
- **L-3** Color legend for non-Gurmukhi/Kirtan divisions.
